...
export function validateEnvAccess(
  ...
  if (!permissions.env?.read) {
    throw new PluginPermissionError(pluginId, "env", "Environment variable access not permitted");
  }
...
export function validateFilesystemAccess(
  ...
  if (!fsPerms) {
    throw new PluginPermissionError(pluginId, "filesystem", "Filesystem access not permitted");
  }

  if (mode === "read" && !fsPerms.read) {
    throw new PluginPermissionError(pluginId, "filesystem", "Filesystem read access not permitted");
  }
  ...
  if (allowedPaths && allowedPaths.length > 0) {
    ...
    if (!isAllowed) {
      throw new PluginPermissionError(pluginId, "filesystem", `Path "${path}" not in allowlist`);
    }
  }
}
