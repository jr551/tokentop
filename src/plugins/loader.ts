...
export async function validatePlugin(plugin: unknown): Promise<PluginValidationResult> {
  ...
  if (p.apiVersion !== CURRENT_API_VERSION) {
    if (typeof p.apiVersion === "number") {
      errors.push(
        `Plugin apiVersion ${p.apiVersion} is not compatible (expected ${CURRENT_API_VERSION})`,
      );
    } else {
...
export async function loadNpmPlugin(
  packageName: string,
  resolvedPath?: string,
): Promise<PluginLoadResult<AnyPlugin>> {
  ...
  if (!isOfficial && !isCommunity && !isScopedCommunity) {
    return {
      success: false,
      error:
        `Invalid npm plugin name "${packageName}". Expected one of:\n` +
