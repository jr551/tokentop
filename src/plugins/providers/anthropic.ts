...
async function discoverClaudeCode(ctx: PluginContext): Promise<CredentialResult> {
  ...
  if (!fileOauth?.accessToken)
    return { ok: false, reason: "missing", message: "No Claude Code credentials found" };
...
export const anthropicPlugin: ProviderPlugin = {
  ...
  async fetchUsage(ctx: ProviderFetchContext): Promise<ProviderUsageData> {
    ...
    const subscriptionType = (credentials.oauth as { subscriptionType?: string } | undefined)
      ?.subscriptionType;
