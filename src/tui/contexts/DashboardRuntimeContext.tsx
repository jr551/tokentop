...
import {
  type ActivityDebugData,
  type ActivityState,
  useEmaActivity,
} from "../hooks/useEmaActivity.ts";
...
export function DashboardRuntimeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const isDiscontinuity =
      lastEntry && (totalTokens < lastEntry.tokens || totalTokens > lastEntry.tokens * 2);
    if (isDiscontinuity) {
      historyRef.current = [];
    }
  ...
  return (
    <DashboardRuntimeContext.Provider value={value}>{children}</DashboardRuntimeContext.Provider>
  );
}
